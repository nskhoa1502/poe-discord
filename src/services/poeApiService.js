...
function printPoeLeague(receivedMessage) {
  CurrentExile.findOneByDiscordUid(receivedMessage.author.id).then(result => {
    if (!result) {
      DiscordHelper.sendMessageAndRemoveCommandMessage(receivedMessage, `!rank is not available for exiles who've not linked their discord accounts to there PoE account.`);
      return;
    }

    PoeHttp.getLeagueLadderForAccount(CURRENT_LEAGUE, result.id).then(body => {

      if (!body) {
        DiscordHelper.sendMessageAndRemoveCommandMessage(receivedMessage, `\`\`\`css\n[Global] There seems to be a problem connecting to api.pathofexile.com ranking unable to be retrieved.\`\`\``);
        return;
      }
      ...
      if (body.entries.length === 1) {
        ...
        msgL += '```';
        DiscordHelper.sendMessageAndRemoveCommandMessage(receivedMessage, msgL);
      }
    }).catch(err => {
      DiscordHelper.sendMessageAndRemoveCommandMessage(receivedMessage, `There was an error retrieving the details from PoE website -> ${err.message}`);
    });
  });
}
