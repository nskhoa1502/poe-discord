...
function processCommand(receivedMessage) {
  ...
  Logger.info(`${receivedMessage.author.username}/${receivedMessage.author.id} entered command: ${primaryCommand}, Arguments: ${args}`);
  ...
  switch (primaryCommand) {
    ...
    case 'members':
      ...
      break;
    case 'filter':
      receivedMessage.channel.send(`Recommended Neversink @ https://github.com/NeverSinkDev/NeverSink-Filter/releases or if you wish to build your own check out https://www.filterblade.xyz/`);
      break;
    case 'admin':
      AdminService.isAdmin(receivedMessage).then(isAdmin => {
        if (!isAdmin) {
        ...
        switch (args[0]) {
          ...
          case 'pending':
            switch (args[1]) {
              case 'remove':
                AppliedExilesService.removePendingGuildInvite(args[2], receivedMessage);
                break;
              default:
                AppliedExilesService.printPendingGuildInvites(receivedMessage);
            }
            break;
          default:
            receivedMessage.channel.send(`The admin command requires extra params`);
        }
      }).catch(err => {
        Logger.error(err);
      });
...
function helpCommand(args, receivedMessage) {
  let command = args[0];
  if (command === 'commands') {
    receivedMessage.channel.send(`Commands: !patch, !filter, !members, !rank, !join`);
    ...
    receivedMessage.channel.send(`Try '!help join' or '!help commands' for a list of commands.`);
  }
}
